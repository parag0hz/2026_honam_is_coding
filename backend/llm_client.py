...
- 키가 없거나 호출이 실패하면 항상 폴백 메시지를 반환해서
...
# 비전(이미지 인식) 모델: 1차 실패 시 2차로 자동 폴백되도록 리스트로 관리
OPENROUTER_VISION_MODELS = [
    os.getenv("OPENROUTER_VISION_MODEL", "anthropic/claude-opus-5"),
    "openai/gpt-5.6-sol",
]
...
def generate_explanation(prompt: str, timeout: float = 10.0) -> str:
    """OpenRouter Chat Completions 를 호출해 자연어 설명을 반환한다 (텍스트 전용)."""
